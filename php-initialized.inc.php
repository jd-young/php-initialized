...
function check_variables($filename, $initialized = array(), $function = "", $class = "", $in_string = false, $tokens = null, $i = 0, $single_command = null) {
	...
	for (; $i < count($tokens); $i++) {
		...
		// function call
			...
			$name = $token[1];
			$class_name = "";
			if (($tokens[$i-1][0] === T_DOUBLE_COLON && $tokens[$i-2][1] === 'self') || ($tokens[$i-1][0] === T_OBJECT_OPERATOR && is_array($tokens[$i-2]) && $tokens[$i-2][1] === '$this')) {
				$class_name = $class;
		...
		if (count($function_calls) === $single_command && ($tokens[$i] === '}' || $tokens[$i] === ';') && !(isset($tokens[$i+1]) && in_array($tokens[$i+1][0], array(T_ELSE, T_ELSEIF, T_CATCH), true))) {
			return $i;
		}
	}
	return $initialized;
